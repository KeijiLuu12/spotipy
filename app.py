...
def top_artist_timeframe_medium():
    ...
    top_artist = recommendation.get_top_artists('medium_term', 5) #top artists of past 6 months

    artist_json = [] # creates list representing json file
    index = 1 #prob can taken out oof 
    for artist in top_artist: #loops through json file for each artist to get imporant data needed from Spotify API like their image and name

        artist_json.append({"id": index, "name": artist[1], "image": artist[2]})
        index += 1
...
def top_artist_timeframe_short():
    ...
    top_artist = recommendation.get_top_artists('short_term', 5)
...
def top_artist_timeframe_long():
    ...
    top_artist = recommendation.get_top_artists('long_term', 5)
